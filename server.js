  ...
  async function playGreetingSequence() {
    ...
    if (greetingAudio) {
      ...
      try {
        ...
        sendTwilioTTS("Hey, congrats on getting started!");
      }
    }
    ...
    if (timeCheckAudio) {
      ...
      try {
        const audio = await synthesizeSpeech(
          "Onboarding takes about 45 minutes. Do you have time right now, or would you rather schedule?"
        );
        timeCheckMarkId = sendAudioToTwilio(audio);
      } catch (err) {
        sendTwilioTTS("Onboarding takes about 45 minutes. Do you have time right now, or would you rather schedule?");
